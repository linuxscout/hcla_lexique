...
def normalize_searchtext(text):
    ...
    return text;
def fr_normalize_searchtext(text):
    text= re.sub(ur'[éèêë]', 'e', text)
    text= re.sub(ur'[àäâ]', 'a', text)
    text= re.sub(ur'[ùûü]' ,'u', text)
    text= re.sub(ur'[ç]', 'c', text)
    text= re.sub(ur'[îï]', 'i', text)
    text= re.sub(ur'[òôö]', 'o', text)
    text=re.sub(ur"( )+"," ",text)
    return text;

def en_normalize_searchtext(text):
    text=re.sub(ur"( )+"," ",text)
    return text;
def detect_language(text):
    if re.search(ur"[\u0621-\u0652]",text):
        return "arabic";
    elif re.search(ur"[éèêëàäâùûüçîïòôö]",text):
        return "french";
    else:
        return "english";
