...
#  Parameters
SEPARATOR=";"
FILE_DATA=u"data/mecanique.csv"
FILE_DB=u"data/mecaniquelexique.db"
DB_CREATE_SQL=u'''create TABLE mecanique
    (
...
def test_select(word):
	...
	conn = sqlite.connect(FILE_DB)
	c = conn.cursor()
	sql = u"select arabic,french,english from mecanique WHERE french_normalized like '%"+word+"%'"
	c.execute(sql)
	list_words=[];
##	row=c.fetchone();
##	while len(row)==0:
...
##		print id,"[%s] '%s'(%s)-%s (%s)"%(type_word,word,str(lenght),word_vocalised,str(lenght_vocalised)),"'%s'"%root,categories;
		...
		c.execute("""insert into mecanique
          values (?,?,?,?,?,?)""",t)
